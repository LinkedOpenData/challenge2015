(function($$, $){
  ...
  $(document).ready(function() {
    ...
    // Note that this should be deleted when we move to foundation_layout
    var $mainNavbar = $('#main_navbar');
    var $navBarInner = $('.navbar-inner');
    var $tosBanner = $('.j-tos-update-banner');
    // TODO: incluse subNav into mainNavbar so we don't have to count them separately.
    var $subNav = $mainNavbar.find('.sub-navbar');

    // Update the main_navbar padding bottom based on the height of the entire nav_bar, including the TOS banner
    var updateMainNavPaddingBottom = function() {
      if ($tosBanner.is(':visible')) {
        $mainNavbar.css('padding-bottom', $navBarInner.height() +
          $tosBanner.height() + $subNav.height());
      } else {
        $mainNavbar.css('padding-bottom', $navBarInner.height() + $subNav.height());
      }
    };
    ...
    var initTOSBanner = function() {
      var $showText = isMobile() ? $('.j-mobile-text') : $('.j-desktop-text');
      var displayCount = parseInt(cookie("tos_update_banner_shows"), 10);
      displayCount = isNaN(displayCount) ? 0 : displayCount;
      var numToShow = isMobile() ? 1 : 2;
      if ($tosBanner.length && displayCount < numToShow) {
        $tosBanner.show();
        $showText.show();
        updateMainNavPaddingBottom();
        cookie('tos_update_banner_shows', (displayCount + 1), {'expires': 365, 'path': '/'});
    ...
    $(window).resize(function(){
      // Resize the navbar and content to the correct positions
      updateMainNavPaddingBottom();
    });
  ...
  $(window).load(function() {
    if(typeof MessageBar !== "undefined"){
      ...
      if(li_msg.length > 0 && cookie("pro-bar-closed") !== "true") {
        var cta_shown;
        ...
        li_bar = new MessageBar({
          showCallBack: function(){
            slideshare_object.ga('pro_bar', cta_shown + '_cta_load');
          },
          hideCallBack: function(){
            cookie("pro-bar-closed", true, { path: '/', expires: 3 });
            slideshare_object.ga('pro_bar', cta_shown + '_cta_hidden');
          }
        });
        li_bar.show();
      }
    ...
    //setting up ab tracking for upload-cta in navbar 
    (function(){
      if(!(slideshare_object.top_nav && slideshare_object.top_nav.visible && slideshare_object.utils)){
        return;
      }
      var Experiments = slideshare_object.utils.imports('Experiments'),
      experiments = new Experiments();
      var gaCategory = "interactive_upload", exp_key = "upload-nav-ab", gaAction = "upload-nav-ab2",
        $create_cta = $('a.create-icn'), $upload_cta = $('a.upload-cta');
      //tracking loads of this version of navbar
      experiments.ga(exp_key, gaCategory, gaAction, "nav_load");
      //tracking upload-cta click of this version of navbar
      $upload_cta.click(function(){
        experiments.ga(exp_key, gaCategory, gaAction, "nav_upload_click");
      });
      //tracking create-cta click of this version of navbar
      $create_cta.click(function(){
        experiments.ga(exp_key, gaCategory, gaAction, "nav_create_click");
      });
    })();
  });
...
/**
  * Returns whether a page can initiate the login modal in the <curWin> the
  * current frame or in any of its ancestor frames, up to <level> levels.
  * Mostly used for embeds actions as they are encapsulated in iframes.
  * @params curWin [DOM] frame window
  * @params level [Integer] level of the frame window
  * @returns the logged_out_action_modal_player_click method so you can call it
  * with your selected params.
  */
  slideshare_object.canInitiateLoginModalInAncestorFrame = function(curWin, level) {
    if (level >= 0) {
      try {
        if (curWin.initiate_login_modal) {
          return curWin.slideshare_object.logged_out_action_modal_player_click;
        }
        return this.canInitiateLoginModalInAncestorFrame(curWin.parent, level - 1);
      } catch(err) {}
    }
    return;
  };
    ...
    initCrossDomainMessaging: function() {
      ...
      if ($.isFunction($.receiveMessage)) {
        $.receiveMessage(
          ...
          (function() {
            var protocol = "https";
            return protocol + '://'+ $(location).attr('hostname');
          }())
        );
      }
    },
