...
header("Content-Disposition: attachment; filename=basetechnology-db.csv");


	$db = LodDb::getInstance();
//	$rows = $db->executeQueryRaw("select id, name, abstract, application_name, copyright, license from application_2014 order by id ASC;");
	$rows = $db->executeQueryRaw("select * from basetechnology_2014 order by id ASC;");
